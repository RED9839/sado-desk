...
2. 첫 실행 → **에셋 가져오기** 창:
   ...
   - **지원 앱플레이어**: 뮤뮤 12(꺼져 있으면 자동 실행) · 뮤뮤 구버전 · LD플레이어(9/14, ldconsole 경유라 다른 앱플레이어와 포트가 겹쳐도 됨) · 블루스택 5, 그 외 adb가 붙는 것. 앱플레이어를 켠 상태에서 "다시 검색"하면 트릭컬 데이터가 있는 기기를 골라 줍니다. 블루스택은 설정 → 고급 → **Android 디버그 브리지(ADB)** 를 켜야 합니다(꺼져 있으면 목록에 'ADB 꺼져 있음'으로 표시). 자동 검색이 안 되면 adb.exe 경로나 포트(예 `5555`)를 직접 입력.
