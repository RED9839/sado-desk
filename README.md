...
> 앱을 처음 실행하면 **사용자 PC의 앱플레이어(뮤뮤·LD플레이어·블루스택·녹스 등)에 설치된 트릭컬 리바이브**에서 직접 추출하는 화면이 뜹니다. 추출물은 본인 PC에서만 쓰고 재배포하지 마세요.
...
2. 첫 실행 → **에셋 가져오기** 창:
   ...
   - **지원 앱플레이어**: 뮤뮤 12(꺼져 있으면 자동 실행) · 뮤뮤 구버전 · LD플레이어 4/9 · 블루스택 4/5 · 녹스 · MEmu, 그 외 adb가 붙는 것. 앱플레이어를 켠 상태에서 "다시 검색"하면 트릭컬 데이터가 있는 기기를 골라 줍니다. 블루스택은 설정 → 고급 → **Android 디버그 브리지(ADB)** 를 켜야 보입니다. 자동 검색이 안 되면 adb.exe 경로나 포트(예 `5555`)를 직접 입력.
