...
(async () => {
  ...
  host.on("extract:done", (r) => {
    ...
    // 사용자가 스스로 누른 중단이 "종료됨 (code null)"로 뜨면 고장 난 것처럼 보인다.
    // 일부만 받아 온 경우도 실패가 아니다 — 사도는 이미 뜬다
    line(r.cancelled ? "중단했습니다. 다시 시작하면 이미 받은 것은 건너뜁니다."
       : r.ok ? "완료! 캐릭터가 화면에 나타납니다. 이 창은 닫아도 됩니다."
       : r.partial ? "일부만 가져왔어요 — 사도는 뜨지만 빠진 것이 있습니다. 다시 시작하면 빠진 것만 받습니다."
       : `끝내지 못했어요 (code ${r.code}) — 위 기록을 확인해 주세요`,
       r.cancelled ? "info" : r.ok ? "ok" : r.partial ? "warn" : "error");
    refresh();
  });
  refresh(); scan();
})();
