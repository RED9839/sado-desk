...
const RULES = [
  {
    ...
    re: /(죽었|죽는다|죽을 뻔|죽어 버|죽여 버|죽음을|죽겠|죽여주|목숨을 걸|목숨을 잃|사망|시체|송장|장례식|저승|영면에 들|늙어 죽)/,
    only: [],
  },
  {
    id: "남성",
    why: "지성체는 전부 여성이다. 영감·아저씨·아빠·오빠·사내 같은 남성 지칭이 성립하지 않는다.",
    re: /(영감님|촌장 영감|아저씨|아빠|아버지|오라버니|오빠|사내|남자|소년|삼촌|할아버지|할아범|아들)(?<!받아들|아들이|알아들|들어들)/,
  ...
  // ---- 종족 규칙 (race 를 가진 규칙은 그 종족에게만 건다) ----
  {
    ...
    re: /(변신|브레스|불을 뿜|용으로 변|를 뿜었|을 뿜었)/,
  ...
  },
  {
    ...
    re: /(배가 고파|배고파|허기|졸려|졸음|잠이 와|자야겠|잠을 자야|눈 뜨자마자|잠에서 깨|자고 일어|눈이 스르르 감)/,
    race: "유령", only: [],
  },
  {
    id: "마녀입맛",
  ...
  // ---- 표기 ----
  {
    ...
    re: /(모나티움|모나디움|모나티엄시|에르피안|에르핀랜드|벨리티안|벨리티엠|유령숲|유령의 숲|정령 산|정령 호수|용족 산)/,
    only: [],
  },
  {
    id: "인간중심",
    why: "엘리아스에 인간은 교주뿐이다. '인류'·'인간의 ○○' 같은 말이 사도 입에서 나올 수 없다.",
    re: /(인류|인간의 |인간들)/,
    only: ["elena", "amelia", "ameliar41", "mute", "haley", "haleysane", "hilde", "rohne", "rohnemayor", "taida", "canna", "nicole", "orr", "lazy", "meluna", "eisia", "heidi", "silvia", "renewaawaken", "renewa_alba"],
  },
  {
    id: "지구교통",
    why: "엘리아스의 탈것은 모나티엄 모노레일과 슈팡의 세그웨이 정도다. 버스·지하철·택시는 없다.",
    re: /(버스(?!킹)|지하철|전철|택시|기차|비행기)/,
    only: ["elena", "amelia", "mute", "rohne", "rohnemayor", "taida", "shoupan", "nicole", "heidi", "meluna"],
  },
];
